...
// Tue Feb  3 15:23:42 2026 by ROOT version 6.38.00
// from TTree TreeB/Output TTree
// found on file: TreeB.root
//////////////////////////////////////////////////////////

#ifndef bkgPreselect_h
#define bkgPreselect_h
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   bkgPreselect(TTree *tree=0);
   virtual ~bkgPreselect();
...
#ifdef bkgPreselect_cxx
bkgPreselect::bkgPreselect(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("TreeB.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("TreeB.root");
      }
...
bkgPreselect::~bkgPreselect()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t bkgPreselect::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t bkgPreselect::LoadTree(Long64_t entry)
{
...
void bkgPreselect::Init(TTree *tree)
{
...
void bkgPreselect::Show(Long64_t entry)
{
...
Int_t bkgPreselect::Cut(Long64_t entry)
{
