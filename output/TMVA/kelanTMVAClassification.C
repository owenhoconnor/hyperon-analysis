...
int TMVAClassification( TString myMethodList = "" )
{
   ...
   TString signalFile = "./TreeS.root";
   TString backgroundFile = "./TreeB.root";

   std::unique_ptr<TFile> signalInput{TFile::Open(signalFile)};
   std::unique_ptr<TFile> backgroundInput{TFile::Open(backgroundFile)};
   if (!signalInput || signalInput->IsZombie())
   {
      throw std::runtime_error("ERROR: could not open signal data file");
   }
   if (!backgroundInput || backgroundInput->IsZombie())
   {
      throw std::runtime_error("ERROR: could not open background data file");
   std::cout<<"--- TMVAClassification       : Using input files: "<<signalFile->GetName()<<" and "<<backgroundFile->GetName()<<std::endl;
   }

   // Register the training and test trees

   TTree *signalTree = (TTree*)signalInput->Get("TreeS");
   TTree *background = (TTree*)backgroundInput->Get("TreeB");
   ...
   TMVA::DataLoader *dataloader = dataloader_raii.get();
   ...
   dataloader->AddVariable("fRecoVertexX", 'F');
   dataloader->AddVariable("fRecoVertexY", 'F');
   dataloader->AddVariable("fRecoVertexZ", 'F');
   dataloader->AddVariable("MomentumX", 'F');
   dataloader->AddVariable("MomentumY", 'F');
   dataloader->AddVariable("MomentumZ", 'F');
   ...
   dataloader->AddSpectator( "NeutrinoX", 'F' );
   dataloader->AddSpectator( "NeutrinoY", 'F' );
   dataloader->AddSpectator( "NeutrinoZ", 'F' );
   dataloader->AddSpectator( "nPFParticles", 'F' );
   dataloader->AddSpectator( "nPrimaryChildren", 'F' );
   dataloader->AddSpectator( "childTrackLengths", 'F' );
   dataloader->AddSpectator( "TrackStartX", 'F' );
   dataloader->AddSpectator( "TrackStartY", 'F' );
   dataloader->AddSpectator( "TrackStartZ", 'F' );
   dataloader->AddSpectator( "TrackEndX", 'F' );
   dataloader->AddSpectator( "TrackEndY", 'F' );
   dataloader->AddSpectator( "TrackEndZ", 'F' );
   dataloader->AddSpectator( "trueP", 'F' );
   dataloader->AddSpectator( "truePDG", 'F' );
   dataloader->AddSpectator( "longestID", 'F' );
