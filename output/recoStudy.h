...
#ifdef recoStudy_cxx
recoStudy::recoStudy(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("/data/ooconnor/sbnd/hyperons/preselection_output/signalDef_output_sig.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("/data/ooconnor/sbnd/hyperons/preselection_output/signalDef_output_sig.root");
      }
...
Int_t recoStudy::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t recoStudy::LoadTree(Long64_t entry)
{
...
void recoStudy::Init(TTree *tree)
{
...
void recoStudy::Show(Long64_t entry)
{
...
Int_t recoStudy::Cut(Long64_t entry)
{
