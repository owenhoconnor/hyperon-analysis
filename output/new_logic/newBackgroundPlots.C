...
constexpr int kNModeCategories =
    kMaxMode - kMinMode + 1;
...
)
{
    ...
    const int nChargedPions =
        nPiPlus + nPiMinus;

    const int nPions =
        nChargedPions + nPiZero;
...
)
{
    ...
    for (size_t i = 0; i < n; ++i)
    {
        ...
        //
        // Decay products are deliberately NOT included when defining
...
void newBackgroundPlots::Loop()
{
    ...
    // ------------------------------------------------------------------------
    // E_nu distributions separated by interaction mode
    // ------------------------------------------------------------------------
    ...
    for (
        int modeIndex = 0;
        modeIndex < kNModeCategories;
        ++modeIndex
    ) {

        hNuEnergyByMode.at(modeIndex) =
            new TH1F(
                Form(
                    "hNuEnergyMode_%d",
                    modeIndex
    ...
    for (
        ...
        hNuEnergyByTopology.at(topology) =
            new TH1F(
                Form(
                    "hNuEnergyTopology_%d",
                    topology
                ),
                "",
                30,
    ...
    )
    {
        ...
        )
        {
            ...
            std::cout
                << "WARNING: invalid chosen MCTruth index "
                << chosenTruthIdx
                << " in tree entry "
                << jentry
                << std::endl;
        ...
        const int intMode =
            trueIntMode->at(chosenTruthIdx);


        const int intType =
            trueIntType->at(chosenTruthIdx);


        const int ccnc =
            trueCCNC->at(chosenTruthIdx);


        const float nuEnergy =
            trueNuEnergy->at(chosenTruthIdx);
    ...
    std::cout
        << "Entries skipped because of bad chosenMCchosenTruthIdx: "
        << nBadTruthIndices
        << std::endl;
    ...
    hTrueCCNC->SetFillColorAlpha(
        fillColour,
        0.7
    );

    hTrueCCNC->SetLineColor(
        kBlack
    );

    hTrueCCNC->SetLineWidth(
        2
    );
    ...
    cCCNC->Print(
        "plots/new_logic/trueBeamCCNC_new.png"
    );
    ...
    hTrueIntMode->SetFillColorAlpha(
        fillColour,
        0.7
    );

    hTrueIntMode->SetLineColor(
        kBlack
    );

    hTrueIntMode->SetLineWidth(
        2
    );
    ...
    cMode->Print(
        "plots/new_logic/trueBeamIntMode_new.png"
    );
    ...
    hTrueIntType->SetFillColorAlpha(
        fillColour,
        0.7
    );

    hTrueIntType->SetLineColor(
        kBlack
    );
    ...
    cIntType->Print(
        "plots/new_logic/trueBeamIntType_new.png"
    );
    ...
    hTrueNuEnergy->SetFillColorAlpha(
        fillColour,
        0.7
    );


    hTrueNuEnergy->SetLineColor(
        kBlack
    );
    ...
    cNuEnergy->Print(
        "plots/new_logic/trueNuEnergy_new.png"
    );
    ...
    for (
        ...
        if (
            topologyCounts.at(topology) > 0
        ) {
            populatedTopologies.push_back(
                topology
            );
        }
    }
    ...
    cTopology->SetLeftMargin(
        0.12
    );

    cTopology->SetRightMargin(
        0.05
    );

    cTopology->SetBottomMargin(
        0.30
    );


    hTopology->SetTitle(
        "Background Final-State Topology Distribution After Preselection"
    );
    ...
    hTopology->SetFillColorAlpha(
        fillColour,
        0.7
    );

    hTopology->SetLineColor(
        kBlack
    );

    hTopology->SetLineWidth(
        2
    );
    ...
    cTopology->Print(
        "plots/new_logic/finalStateTopologiesBkg_new.png"
    );
    ...
    THStack* hsModeVsNuE =
        new THStack(
            "hsModeVsNuE",
            "Surviving Background by Interaction Mode;True E_{#nu} [GeV];Interactions"
        );
    ...
    cModeVsNuE->Print(
        "plots/new_logic/modeVsNuEnergyBkg_new.png"
    );
    ...
    THStack* hsTopologyVsNuE =
        new THStack(
            "hsTopologyVsNuE",
            "Surviving Background by Final-State Topology;True E_{#nu} [GeV];Interactions"
        );
    ...
    cTopologyVsNuE->SetRightMargin(
        0.30
    );


    hsTopologyVsNuE->Draw(
        "HIST"
    );
    ...
    cTopologyVsNuE->Print(
        "plots/new_logic/topologyVsNuEnergyBkg_new.png"
    );
    ...
    cModeVsTopology->SetRightMargin(
        0.15
    );

    cModeVsTopology->SetBottomMargin(
        0.30
    );

    cModeVsTopology->SetLeftMargin(
        0.15
    );
    ...
    cModeVsTopology->Print(
        "plots/new_logic/modeVsTopologyBkg_new.png"
    );
